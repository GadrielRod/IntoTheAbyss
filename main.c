...
int combate_executar(Jogador* j, Entidade* monstro) {
    ...
    if (vitoria) {
        printf("\n>> VOCE DERROTOU %s!\n", monstro->nome);
        // Habilidade Sacerdote: Cura pós-combate
...
int main() {
    ...
    while (jogando && jogador->base->hp_atual > 0) {
        ...
        break;
            ...
            Entidade* boss = entidade_criar("LICH REI ALDOUS", CLASSE_MONSTRO, 60, 8, 14, 7, 3);
